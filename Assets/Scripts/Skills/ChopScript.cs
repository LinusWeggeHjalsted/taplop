...
public class ChopScript : MonoBehaviour, SkillScript
{
    ...
    public int EnemyPriority(Vector3 fromPosition, GameObject enemy)
    {
        ...
        {
            if ((float)enemyScript.CurrentHealth > ((float)enemyScript.MaxHealth / 2))
            {
                return 2;
            }
            else
    ...
    public void UseSkill(Vector3 targetPosition, GameObject wielder)
    {
        ...
        if (target != null)
        {
            ...
            if ((float)wielderScript.CurrentHealth > ((float)wielderScript.MaxHealth / 2))
            {
    ...
    void Awake()
    {
        ...
        description = "Attack target, dealing 1.5x damage if you have more than half health";
        range = 1f;
        radius = 0;
        distance = 0;
        skillDuration = 0;
        stunDuration = 0;
        cooldown = 0;
        skillSprite = Resources.Load<Sprite>("Skills/Chop");
    }
